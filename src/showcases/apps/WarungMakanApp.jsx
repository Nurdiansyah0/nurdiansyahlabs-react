        ...
        <div style={{ fontFamily: '"Inter",sans-serif', minHeight: '100vh', background: '#f5f5f5' }}>
            ...
            <div style={{ background: '#1c2125', minHeight: '60vh', display: 'flex', position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ position: 'relative', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ textAlign: 'center', background: 'rgba(28, 33, 37, 0.7)', padding: '4rem 5rem', borderRadius: '30px', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.15)' }}>
                        ...
                        <button aria-label="Action button" onClick={() => document.getElementById('menu').scrollIntoView({ behavior: 'smooth' })} style={{ background: BRAND.primary, color: '#111', border: 'none', padding: '16px 40px', borderRadius: '30px', fontWeight: 900, cursor: 'pointer', fontSize: '1.2rem', boxShadow: '0 4px 15px rgba(249, 168, 38, 0.4)' }}>Lihat Menu Siomay</button>
                    </div>
                </div>
            </div>
            {/* About */}
            ...
            <div id="menu" style={{ background: '#f5f5f5', padding: '5rem 2rem' }}>
                ...
                <div style={{ display: 'flex', gap: '15px', justifyItems: 'center', justifyContent: 'center', flexWrap: 'wrap', marginBottom: '4rem' }}>
                    {CATS.map(c => (
                        <button aria-label="Action button" key={c} onClick={() => setCat(c)} style={{ padding: '12px 28px', borderRadius: '30px', border: 'none', background: cat === c ? BRAND.primary : '#fff', color: cat === c ? '#111' : '#444', fontWeight: cat === c ? 800 : 700, cursor: 'pointer', fontSize: '1rem', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', transition: '0.2s' }}>
                            {c}
                        </button>
                    ))}
                </div>
                        ...
                        <div key={item.id} style={{ background: '#fff', borderRadius: '24px', overflow: 'hidden', boxShadow: '0 10px 30px rgba(0,0,0,0.08)', transition: 'transform 0.3s ease', cursor: 'default' }}>
                            ...
                            <div style={{ padding: '2rem' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                                    <h2 style={{ fontSize: '1.3rem', fontWeight: 800, color: '#1c2125', margin: 0, flex: 1 }}>{item.name}</h2>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '1rem', borderTop: '1px solid #eee' }}>
                                    ...
                                    <button aria-label="Action button" onClick={() => orderViaWA(item)} style={{ background: '#25d366', color: '#fff', border: 'none', padding: '12px 24px', borderRadius: '14px', cursor: 'pointer', fontWeight: 800, fontSize: '1rem', display: 'flex', alignItems: 'center', gap: '8px', boxShadow: '0 4px 15px rgba(37, 211, 102, 0.4)' }}>
